---
layout: mypost
title: Appium自动翻页查找所需要的内容
categories: [Python]
---

### unittest创建一个Appium连接实例
Python的unittest中继承unittest.TestCase后，即可在运行实例的时候自动运行setUp方法，结束时调用tearDown。  
...
```
### 滑动到页面顶端和底端判断
跟据一个可以上下滑动的列表特点，可知页面列表中各个元素resource-id一般一致，所以我们确定resource-id的情况下只需要判断不同的文本
信息则可以判断是否滑动到了页面底端或者顶端。  
判断是否滑动到顶端：  
